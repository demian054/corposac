 ...
 * */
class personas_model extends MY_Crud_model {
    ...
    function getAll($limit, $start, $eliminado = '0', $get_count = false) {
        ...
        $_cedula = $this->input->post('cedula');
        if ($_cedula) {
           $this->db->where('cedula_rif LIKE ', '%'.addslashes($_cedula).'%') ;
        }
        $_cc = $this->input->post('cc');
        if ($_cc) {
           $this->db->where('cuenta_contrato LIKE ', '%'.addslashes($_cc).'%') ;
        }
